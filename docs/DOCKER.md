...
services:
  armada-server:
    image: jchristn77/armada-server:v0.5.0
    ports:
      - "7890:7890"
      - "7891:7891"
      - "7892:7892"
  ...
  armada-dashboard:
    image: jchristn77/armada-dashboard:v0.5.0
    ports:
      - "3000:80"
    environment:
...
This produces both `jchristn77/armada-server:latest` and `jchristn77/armada-server:v0.5.0` (and the same for the dashboard).
