    ...
    public class DatabaseTestRunner
    {
        ...
        private async Task TestCaptainCrudAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                Captain captain = await fixture.CreateCaptainAsync(tenant.Id, user.Id, "operational-captain", token: token, model: "gpt-5.4").ConfigureAwait(false);
                ...
                DatabaseAssert.Equal("gpt-5.4-mini", updated.Model, "Captain.Model");
        ...
        private async Task TestMissionCrudAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                Mission mission = await fixture.CreateMissionAsync(tenant.Id, user.Id, voyage.Id, vessel.Id, captain.Id, "operational-mission", token: token, totalRuntimeMs: 9000).ConfigureAwait(false);
                ...
                DatabaseAssert.Equal(9000L, read.TotalRuntimeMs ?? -1L, "Mission.TotalRuntimeMs");
                ...
                DatabaseAssert.Equal(12000L, updated.TotalRuntimeMs ?? -1L, "Mission.TotalRuntimeMs");
            }
            finally
            {
                await fixture.CleanupAsync(token).ConfigureAwait(false);
