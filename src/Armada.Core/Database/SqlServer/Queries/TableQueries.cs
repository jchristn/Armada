    ...
    public static class TableQueries
    {
        ...
        public static List<SchemaMigration> GetMigrations()
        {
            ...
            return new List<SchemaMigration>
            {
                ...
                new SchemaMigration(
                    14,
                    "Add model to captains for specifying AI model",
                    @"
                    IF COL_LENGTH('captains', 'model') IS NULL
                        ALTER TABLE captains ADD model NVARCHAR(450);"
                ),
                new SchemaMigration(
                    15,
                    "Add total_runtime_seconds to missions",
                    @"
                    IF COL_LENGTH('missions', 'total_runtime_seconds') IS NULL
                        ALTER TABLE missions ADD total_runtime_seconds FLOAT;"
                )
            };
        }
